...
///
/// Provides structures and functions for accessing the database.
/// This includes both custom SQL-queries as well as predefined
/// queries for listing databases and tables.
use tauri::State;
use tokio::select;
...
use crate::util::{self, log_error, log_info};

const COMPONENT_ID: &str = "db";
...
pub mod commands {
    ...
    /// Check that a (postgres-) database can be reached using the given
    /// connection settings.
    ///
    #[tauri::command]
    ...
    pub async fn db_query(
        ...
        util::log_info(COMPONENT_ID, "called: db_query");
        {
            ...
            match sender.send(query).await {
                ...
                Err(_) => {
                    ...
                    util::log_error(COMPONENT_ID, format!("{}", failure_msg).as_str());
        ...
        }
        {
            ...
            if let Some(query_result) = receiver.recv().await {
                util::log_info(COMPONENT_ID, "sending db result to frontend");
...
fn try_get_field_as_string(row: &tokio_postgres::Row, index: usize) -> Result<String, ()> {
    ...
    match field_type {
        ...
        some_type => {
            util::log_error(
                COMPONENT_ID,
                format!("Could not convert column of type: {:?}", some_type).as_str(),
            );
            Err(())
        }
    }
...
async fn run_standalone_query(
    ...
    log_info(
        COMPONENT_ID,
        format!("Connection-String: \"{}\"", connection_str).as_str(),
    );
    ...
    log_info(COMPONENT_ID, format!("Query: \"{}\"", query).as_str());
...
pub async fn db_task(
    ...
    while let Some(db_query) = channel_to_db_rx.recv().await {
        log_info(
            COMPONENT_ID,
            format!("Received db query: {:?}", db_query).as_str(),
        );
        ...
        match table_data {
            Ok(rows) => {
                ...
                if let Err(_) = channel_to_tauri_tx.send(database_result).await {
                    log_error(
                        COMPONENT_ID,
                        format!("Could not return results to caller. Exit task.").as_str(),
                    );
            ...
            Err(_) => {
                log_info(
                    COMPONENT_ID,
                    format!("Error executing query - no results").as_str(),
                );
